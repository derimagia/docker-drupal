...
/**
 * @file
 * Contains \DockerDrupal\composer\ScriptHandler.
 */

namespace DockerDrupal\composer;
...
class ScriptHandler {
  ...
  public static function createRequiredFiles(Event $event) {
    ...
    if (!$fs->exists($cwd . '/config')) {
      $oldmask = umask(0);
      $fs->mkdir($cwd . '/config', 0777);
      umask($oldmask);
      $event->getIO()->write("Create a config directory with chmod 0777");
    }
  }
